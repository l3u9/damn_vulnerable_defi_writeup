...
describe('[Challenge] The rewarder', function () {
    ...
    it('Execution', async function () {
        const Contract = await ethers.getContractFactory('TheRewarderExploit');
        const contract = await Contract.connect(player).deploy(TOKENS_IN_LENDER_POOL, rewarderPool.address, flashLoanPool.address, rewardToken.address, liquidityToken.address);
        // for (let i = 0; i < users.length; i++) {
        //     await rewarderPool.connect(users[i]).distributeRewards();
        ...
        // }
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        console.log("before exploit totalsupply",await rewardToken.totalSupply())
        // console.log(await contract.test())
        // console.log(await contract._balance());
        console.log((await contract.exploit()));
        console.log("after exploit totalsupply",await rewardToken.totalSupply())
    });
    ...
    after(async function () {
        ...
        console.log("rewardToken totalSupply:", await rewardToken.totalSupply())
